...
class EMSController extends Controller
{
    public function index()
    {
        $transaction = Transaction::orderBy('transactions.id', 'DESC')
                    ...
                    ->where('trandesc.section_id', '=', 2)
                    ->paginate(10);
       
        $status = Status::all();
        $applicant = Applicant::all();
        $trandesc = Trandesc::where('section_id', '=', 2)->get();

        
        return view('transactions/EMS/transaction')
    ...
    public function store(Request $request)
    {
        ...
        return redirect('/EMStransactions');
    }
    ...
    public function show($id)
    {
        ...
        return view('transactions/EMS/showTran')
        ->with('transaction' ,$transaction)
    ...
    public function edit($id)
    {
        ...
        return view('transactions/EMS/editTran')
        ->with('transaction', $transaction)
        ->with('status', $status)
        ->with('trandesc', $trandesc)
        ->with('applicant', $applicant);
    ...
    public function update(Request $request, $id)
    {
        ...
        $tran_history->save();
            ...
            return redirect('/EMStransactions')->with('status','Transaction Updated');
    ...
    public function destroy($id)
    {
        ...
        return  redirect('/EMStransactions')->with('status', 'Transaction Deleted');
