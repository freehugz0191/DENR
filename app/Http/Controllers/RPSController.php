...
class RPSController extends Controller
{
    public function index()
    {
        $transaction = Transaction::orderBy('transactions.id', 'DESC')
                    ...
                    ->where('trandesc.section_id', '=', 3)
                    ->paginate(10);
       
        $status = Status::all();
        $applicant = Applicant::all();
        $trandesc = Trandesc::where('section_id', '=', 3)->get();

        
        return view('transactions/RPS/transaction')
    ...
    public function store(Request $request)
    {
        ...
        return redirect('/RPStransactions');
    }
    ...
    public function show($id)
    {
        ...
        return view('transactions/RPS/showTran')
        ->with('transaction' ,$transaction)
    ...
    public function edit($id)
    {
        ...
        return view('transactions/RPS/editTran')
        ->with('transaction', $transaction)
        ->with('status', $status)
        ->with('trandesc', $trandesc)
        ->with('applicant', $applicant);
    ...
    public function update(Request $request, $id)
    {
        ...
        $tran_history->save();
            ...
            return redirect('/RPStransactions')->with('status','Transaction Updated');
    ...
    public function destroy($id)
    {
        ...
        return  redirect('/RPStransactions')->with('status', 'Transaction Deleted');
